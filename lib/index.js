...
function Tineye (public, private) {
    ...
    self.sign = function (args) {
        ...
        if (typeof args.qs !== 'undefined') {
            var keys = Object.keys(args.qs).sort() // order matters
            for (var i in keys) {
              var property = keys[i]
              var append = property === "image_url"
                  ? encodeURIComponent(args.qs[property])
                  : args.qs[property];

              if(i != 0)
                concat += '&';

              concat += property + '=' + append;
              console.warn("adding prop " + property + " " + append)
            }
        }
        console.warn("signing " + concat + " as " + hmac(concat, self.private))
...
/**
 * Provide general access to the tineye api.
 *
 * @param {Object} HTTP request arguments
 *
 * @return {Object}
 */
Tineye.prototype.request = this.request;
